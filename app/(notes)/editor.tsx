...
import RichTextEditor from "@chaitrabhairappa/react-native-rich-text-editor";
import type { Block, ContentChangeEvent, ActiveStylesState } from "@chaitrabhairappa/react-native-rich-text-editor";
import { jsonToBlocks, blocksToJson, EMPTY_BLOCKS } from "../../src/lib/noteContent";
...
export default function NotesEditorScreen() {
    ...
    getNote(Number(id))
      .then((n) => {
        ...
        if (n) {
          ...
          if (content && !isLexicalJson(content) && !jsonToBlocks(content).length && /<[a-z][\s\S]*>/i.test(content)) {
            try {
          ...
          setBlocks(jsonToBlocks(content));
          setColor(n.color);
  ...
  const handleSave = useCallback(async () => {
    ...
    setIsSaving(true);
    try {
      const content = blocks.length > 0 ? blocksToJson(blocks) : "";
      if (noteId) {
  ...
  }, [isSaving, noteId, title, blocks, color, isPinned, router, haptics, t]);
  ...
  const handleContentChange = useCallback((event: ContentChangeEvent) => {
    if (suppressDirtyRef.current) {
      suppressDirtyRef.current = false;
      return;
    }
    setBlocks(event.nativeEvent.blocks);
    setIsDirty(true);
  }, []);

  const handleActiveStylesChange = useCallback((_styles: ActiveStylesState) => {
    // Reserved for custom toolbar integration if needed
  }, []);
      ...
      >
        <View className="flex-1 bg-background">
          <View className="w-full max-w-md self-center gap-4 p-4 pb-8">
            ...
            <View className="rounded-xl border border-border bg-card min-h-[200px]">
              <RichTextEditor
                ...
                variant="plain"
                showToolbar={true}
                toolbarOptions={["bold", "italic", "strikethrough", "underline", "bullet", "numbered", "checklist"]}
              />
            </View>
          </View>
        </View>
